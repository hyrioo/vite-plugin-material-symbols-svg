...
export default function materialSymbolsSvg(iconsDefOrPath: IconsInput | string, opts: MaterialSymbolsPluginOptions = {}): Plugin {
    ...
    let iconsDef: IconsInput | undefined;
    if (typeof iconsDefOrPath !== 'string') {
        iconsDef = iconsDefOrPath;
    }
    ...
    return {
        ...
        async configResolved(config) {
            ...
            if (typeof iconsDefOrPath === 'string') {
                const absolutePath = path.isAbsolute(iconsDefOrPath)
                    ? iconsDefOrPath
                    : path.resolve(root, iconsDefOrPath);
                
                try {
                    const module = await import(absolutePath);
                    iconsDef = module.default;
                } catch (e) {
                    const msg = e instanceof Error ? e.message : String(e);
                    throw new Error(`[material-symbols-svg] Failed to load icons definition from ${absolutePath}: ${msg}`);
                }
            }
